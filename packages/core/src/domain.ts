...
export function buildBusinessGlossary(config: BrainConfig): string {
  ...
  return [
    ...
    '  Use it for "today", "this month", and every date range filter.',
    ...
    "     Holds header records for Sales ([InvSR] = 'SIN'), Purchase ([InvSR] IN ('PIN','PIO')), Sale Return/Credit Note ([InvSR] IN ('SRC','CNO','CNI')), Purchase Return/Debit Note ([InvSR] IN ('PTD','DNI','DNO')), and Proforma Invoices.",
    ...
    '     Rate = [Rate], Line Amount = [Amount], GST Assessable / Taxable Value = [TaxOnAmt] (the base value on which GST is calculated), GST Taxes = [CGSTAmt] (CGST), [SGSTAmt] (SGST), [IGSTAmt] (IGST), [GSTCessAmt] (GST Cess), Total Line GST = (ISNULL([CGSTAmt], 0) + ISNULL([SGSTAmt], 0) + ISNULL([IGSTAmt], 0) + ISNULL([GSTCessAmt], 0)), Extra/Overhead Amount = [ExtraAmt] (holds total line overhead expenses including GST tax), Total Line Amount = ([Amount] + ISNULL([ExtraAmt], 0)), Free Qty = [STFreeQty].',
    ...
    '    - Sequence / Line No: [GrLedger].[GlIsno] is the unique line number for each [GrTranID].',
    ...
    '',
    '- "bill-wise outstanding" / "due bills" / receivables / payables / adjustments -> [dbo].[GLOS] ([GlTranIdno], [GlVhino], [GlIsno], [GlOsType], [GlOsId], [GlAmount], [GlOsAmt], [GlAccountId], [GlDueDate], [GlDate]):',
...
export function buildFreshnessQuery(config: BrainConfig): string {
  const salesSeries = config.salesInvoiceSeries || 'SIN';
  return "SELECT MAX([EIDocDate]) AS [lastBill], COUNT(*) AS [bills] FROM [dbo].[InvTranTbl] WHERE [InvSR] = '" + salesSeries + "' AND ISNULL([RecStatus], 0) NOT IN (-1, 2)";
}
